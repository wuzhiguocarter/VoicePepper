# Spec: ble-device-management

## Purpose

管理 BLE 录音笔设备的扫描发现、连接/断开、自动重连及设备信息查询。
...
系统 SHALL 允许用户从已发现设备列表中选择一个设备进行连接，连接成功后订阅所有必要的 Characteristic 通知。系统 SHALL 在扫描发现设备时缓存 CBPeripheral 强引用，确保连接时对象不被 ARC 回收。PreferencesView SHALL 通过 EnvironmentObject 获取 AppDelegate 持有的唯一 BLEDeviceManager 实例。
