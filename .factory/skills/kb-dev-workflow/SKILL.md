...
**调用：** `Skill("openspec-propose")`

将用户的原始需求描述传入。openspec-propose 会自动创建 change 并生成全套规格文档（proposal.md + design.md + specs/*.md + tasks.md）。
...
**调用：** `Skill("openspec-apply-change")`

无需传额外参数，openspec-apply-change 会从当前 openspec 状态自动识别待实现的 change。
...
**构建失败处理：** 若 `swift build` 报错，就地修复编译错误（调用 openspec-apply-change 继续修复），再重试构建，不跳过此步骤。
...
2. 收集测试证据清单（供步骤 5 写入 PR body）：
   ...
   ```
3. 若关键测试失败，先修复代码（调用 openspec-apply-change 继续修复），再重新测试，直到核心功能验证通过
...
**调用：** `Skill("openspec-archive-change")`

openspec-archive-change 会：
1. 检查 artifact 完成状态
...
```
若返回结果非空，说明当前分支已有 open PR，则**跳过创建新 PR**——仅执行 commit + push（代码会自动同步到已有 PR）。跳过 `git-commit-push-pr` skill，改为手动执行：
```bash
git add -A
...
**调用前准备：** 先通过 `git-commit-push-pr` 创建 PR（此时 PR body 中截图部分用文件名占位），然后再上传截图并更新 PR body。

**调用：** `Skill("git-commit-push-pr")`

git-commit-push-pr 会自动：
